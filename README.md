# Hi, I'm Juan Manuel! 👋

⚡ **Electrical Engineering Student** | 💡 **Tech Enthusiast & Continuous Learner**

Electrical Engineering Student · Technology · Hardware & Electricity  
I'm an Electrical Engineering student from Colombia, starting my journey into the world of technology, circuits, and coding.  

I enjoy learning how things work from the ground up and applying theory to hands-on projects.  

---

### 🚀 What I'm exploring & working with

**Engineering Fundamentals**
* Circuit Analysis & Basic Electronics
* Electrical Measurements & Instrumentation
* Physics & Applied Mathematics

**Programming & Tools (Getting Started!)**
* **Python** — *Learning basic logic & problem-solving*
* **C / C++** — *Exploring low-level programming & algorithms*
* **MATLAB** — *Using it for math and engineering calculations*

**Software & Workspace**
* Git & GitHub *(Learning to document and share my code)*
* VS Code

---

### 🛠️ Currently learning

* 🐍 Programming fundamentals from scratch
* 🔌 How microcontrollers (like Arduino) interact with code
* 📐 Schematic reading and basic circuit simulation
* ⚡ Electric circuit theory and problem-solving

---

### 💻 Projects & Milestones

* **First Code Projects** — Small Python scripts solving engineering calculations and logic exercises.
* **Basic Circuit Design** — Hands-on breadboard experiments applying Ohm's Law and circuit analysis.

*More projects coming as I keep building and learning!*

---

